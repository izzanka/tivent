            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">
                        ...
                        <hr>
                    @guest
                        <a href="/detail/{{$events->id}}" class="btn btn-primary">Detail Event</a>
                    @else
                        @if ($events->user_id == Auth::user()->id)
                            @if ($events->status_event == 1 || $events->status_event == 2)
                                <a href="/event" class="btn btn-primary">Event</a>
                            @else
                                <a href="/event/{{$events->id}}/edit" class="btn btn-primary">Edit Event</a>
                            @endif
                        @else
                        <a href="/detail/{{$events->id}}" class="btn btn-primary">Detail Event</a>
                        @endif
